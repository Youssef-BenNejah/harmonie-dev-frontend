      ...
      <form className="space-y-4" onSubmit={submit}>
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-2">
            <Label>Prénom</Label>
            <Input required value={form.prenom} onChange={(e) => update({ prenom: e.target.value })} className="h-11 rounded-xl" />
          </div>
          <div className="space-y-2">
            <Label>Nom</Label>
            <Input required value={form.nom} onChange={(e) => update({ nom: e.target.value })} className="h-11 rounded-xl" />
          </div>
        </div>
        <div className="space-y-2">
        ...
        <div className="space-y-2">
          ...
          <Input required value={form.telephone} onChange={(e) => update({ telephone: e.target.value })} className="h-11 rounded-xl" />
        </div>
        <div className="space-y-2">
        ...
        {plans.length > 0 ? (
          <div className="space-y-2">
              ...
              {plans.map((p) => (
                <button
                  ...
                  className={cn(
                    ...
                    planId === p.id ? "border-ocean bg-ice/60" : "border-border bg-background hover:bg-ice/30",
                  )}
                >
                  <span
                    className={cn(
                      "mt-0.5 grid size-4 shrink-0 place-items-center rounded-full border",
                      planId === p.id ? "border-ocean bg-ocean" : "border-border",
                    )}
                  >
                  ...
                  <span>
                    <span className="block text-sm font-semibold text-navy">
                      {p.nom}{" "}
                      {p.freeTrial ? null : <span className="font-normal text-muted-foreground">— {p.prixMensuel} DT/mois</span>}
                    </span>
            ...
            <p className="text-xs text-muted-foreground">Optionnel — notre équipe confirmera le plan lors de la création de votre compte.</p>
          </div>
        ) : null}
